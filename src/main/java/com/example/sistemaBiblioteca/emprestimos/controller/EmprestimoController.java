...
public class EmprestimoController {
    ...
    public ResponseEntity<ClienteComEmprestimosDto> getClienteComEmprestimos(@PathVariable Long clienteId) {
        ...
        List<EmprestimoComLivroDto> emprestimosDoCliente = emprestimoService
                .convertToDtoList(cliente.getHistoricoDeEmprestimos());
        /*
         * ClienteComEmprestimosDto clienteDto = new ClienteComEmprestimosDto();
         * BeanUtils.copyProperties(cliente, clienteDto);
         * clienteDto.setEmprestimos(emprestimosDoCliente);
         */
        ClienteComEmprestimosDto client = clienteService.toDto(cliente);
    ...
    public ResponseEntity<String> realizarDevolucao(@PathVariable Long clienteId, @PathVariable Long livroId) {
        ...
        Optional<EmprestimoModelo> emprestimoOptional = emprestimosRepository
                .findByClienteAndLivroAndDataDevolucaoIsNull(cliente, livro);
        if (emprestimoOptional.isEmpty()) {
            return ResponseEntity.badRequest()
                    .body("Este livro não foi emprestado a este cliente ou já foi devolvido.");
        }
    ...
    /*
     * private List<EmprestimoComLivroDto> mapEmprestimos(List<EmprestimoModelo>
     * emprestimos) {
    ...
    // TODO os nomes dos parametros do modelo e DTO tem que ser iguais
    /*
     * private List<EmprestimoComLivroDto> mapEmprestimos(List<EmprestimoModelo>
     * emprestimos) {
     * ModelMapper modelMapper = new ModelMapper();
     * 
     * modelMapper.addMappings(new PropertyMap<EmprestimoModelo,
     * EmprestimoComLivroDto>() {
     * 
     * @Override
